...
export default class PartnerLists {
  ...
  async getPartners() {
    ...
    return partners.map(partner => {
      ...
      if (!character) throw new Error(`Missing character in data: ${partner.character}.`);
      return {
        id: partner.id,
        name: characterCounts[partner.character] > 1 ? partner.name : character.short || character.name,
