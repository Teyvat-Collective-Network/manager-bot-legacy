...
export async function updateAPI(users) {
  ...
  for (const user of users) {
    ...
    await different && console.log(user.username, apiUser, expected); // client.tcn.editUser(expected).catch(() => {});
  }
}
...
export async function updateRoles(users) {
  ...
  for (const [,guild] of client.guilds.cache) {
    ...
    for (const user of users) {
      ...
      await different && console.log(guild.name, user.username, managed, expected, rest) // member.roles.set(rest.concat(expected)).catch(() => {});
    }
  }
}