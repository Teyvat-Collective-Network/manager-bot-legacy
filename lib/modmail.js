...
export async function relay(interaction, { content, anon, files }) {
  ...
  if (dm) {
    ...
    const participants = (await interaction.client.db.threadParticipant.find({ thread: thread.uuid })).filter(participant => participant.guild !== interaction.guild.id);

    for (const participant of participants) {
      try {
        await interaction.client.channels.cache.get(participant.channel).send({ embeds: [{ title: `Message from ${interaction.guild.name}`, ...embed }] });
      } catch {}
    }

    await interaction.editReply({ embeds: [{ title: `Outgoing Message to ${participants.length} server${participants.length === 1 ? '' : 's'}`, ...embed }]});

    await interaction.client.db.modmailMessage.create({
      thread: thread.uuid,
      type: 'cross-server-message',
      time: new Date(),
      author: interaction.user.id,
      content,
      anon,
